    ...
    selectors.forEach(function (selector) {
      ...
      forEachEls2(
        ...
        function (newEl, i) {
          ...
          var callback = switches2[selector]
            ? switches2[selector].bind(
                this,
                oldEl,
                newEl,
                options,
...
Pjax.prototype = {
  ...
  loadContent: function (html, options) {
    ...
    if (
      ...
      contains2(document, this.options.selectors, document.activeElement)
    ) {
      try {
...
Pjax$1.prototype.getElements = function () {
  let links = Array.from(
    document.querySelectorAll("a:not(.no-pjax):not([data-fancybox])")
  );
  ...
  function handleSelectChange(event) {
    ...
    const fullUrl = serializedData
      ? `${baseActionUrl}?${serializedData}`
      : baseActionUrl;
    document.body.dataset.pgPrev = document.body.dataset.pg;
